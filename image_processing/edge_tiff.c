 ...
 * gcc -std=c99 -ltiff -lm edge_tiff.c
 */
...
Color getColor(int i, int j, ImageRGB* image)
{
  Color c;
  int pos = index(i,j,image->width);
  c.r = TIFFGetR(image->raster[pos]);
  c.g = TIFFGetG(image->raster[pos]);
  c.b = TIFFGetB(image->raster[pos]);
  return c;
}

void setColor(Color c, int i, int j, ImageRGB* image)
{
  int pos = index(i,j,image->width);
...
void processPixels(ImageRGB* input,
		   ImageRGB* output)
{
  for (int i=0; i<input->height; ++i)
  {
    setColor(color(0,0,0), i, 0, output);
    for (int j=1; j<input->width;++j)
    {
      Color c1 = getColor(i,j-1,input);
      Color c2 = getColor(i,j,input);
      
      setColor(colorDiff(c1,c2),
	       i,j, output);
    }
  }
}
